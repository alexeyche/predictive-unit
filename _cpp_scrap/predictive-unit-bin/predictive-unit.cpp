...
int server(const TVector<TString>& argsVec) {
	...
	if (!startData.empty()) {
		NPredUnitPb::TInputData startDataPb;
		std::ifstream ifile(startData);
		ReadProtobufMessage(ifile, &startDataPb);

		TInputData inpData(startDataPb);
		while (!sim.IsSumulationRunning(inpData.SimId)) {};
		sim.IngestDataAsync(inpData);
	}
